...
export const more_seller = () => {
  return async function (dispatch) {
    try {
      let seller = await axios.get(`http://localhost:3001/count`);
      dispatch({ type: MORE_SELLER, payload: seller.data });
    } catch (error) {
      console.error("Error al traer los mas vendidos:", error);
    }
  }
};
